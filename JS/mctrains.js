...
function loadStations(){
    loadStations.array.forEach(station => buildSelect(station));
}
...
function callMcstationsAPI(stationAbbr){
    let url = "http://api.bart.gov/api/etd.aspx?cmd=etd&orig=" + stationAbbr
    + "&json=y&key=" + key; 
    console.log(url);
...
function displayTrain(train){
    ...
    $("#trains").append("<li data-role='list-divider' style='font-size:1.5em;'>" +
    destination + "</li>");
...
function displayArrival(arrival){
    ...
    $("#trains").append("<li><div style='display: flex;'><div class='trainBox' style='background-color: " +
    color + "></div><div class='trainInfo'>" + out + "</div></div>'</li>")
}